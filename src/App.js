...
class App extends Component {
  ...
  async componentDidMount() {
    ...
    const test = `https://cors-anywhere.herokuapp.com/https://api.darksky.net/forecast/bab642085489d446f31df424bdf3e506/${this.state.location.current}`
    const { data: allDays } = data.daily;
    console.log(test)
   
    let week = [...this.state.week];
    //refractor to shorten code here
    const days = [allDays[0], allDays[1], allDays[2],allDays[3],allDays[4], allDays[5], allDays[6]]
    ...
    return (
      <div id="background">
        <div className="row justify-content-center">
          <img src='https://bit.ly/2PPfHJ5' alt=''></img>
      
        </div><Input toggle = {toggle.show} click ={this.handleToggle} loc={this.updateTest} label={location.name}></Input>
        <WeatherGrid week={week} />
        <p>By Branden LaCour, Powered By Dark Sky. 
        </p>
